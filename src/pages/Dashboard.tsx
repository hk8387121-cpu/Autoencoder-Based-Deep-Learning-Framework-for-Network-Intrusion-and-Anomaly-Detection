...
export default function Dashboard() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => active && payload?.length ? <div className="bg-slate-900/90 border border-slate-700 p-3 rounded-lg shadow-xl"><p className="text-slate-300 text-xs mb-2 font-medium">{label}</p>{payload.map((entry: any, index: number) => <p key={index} className="text-sm font-semibold" style={{ color: entry.color }}>{entry.name}: {typeof entry.value === 'number' && entry.value < 1 ? entry.value.toFixed(4) : entry.value}</p>)}</div> : null;
