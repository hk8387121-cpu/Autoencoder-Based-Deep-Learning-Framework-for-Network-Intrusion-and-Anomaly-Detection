...
# Runtime event store. Render's filesystem is ephemeral, so this intentionally
# represents the current service instance rather than pretending to be durable SIEM storage.
...
def resolve_dataset_path(path: str) -> str:
    ...
    requested = Path(path)
    candidates = [
        requested if requested.is_absolute() else BASE_DIR / requested,
        BASE_DIR / "data" / requested.name,
        BASE_DIR / "data" / "nsl_kdd_subset.csv",
    ]
...
def start_background_training():
    ...
    thread = threading.Thread(
        ...
        kwargs={"dataset_path": "data/nsl_kdd_subset.csv", "normal_label": "normal", "percentile": 95.0},
        name="autoencoder-initial-training",
        daemon=True,
    )
    thread.start()
    return True
...
def report_summary():
    ...
    return {
        ...
        "total_predictions": metric_snapshot["total_predictions"],
        "anomalies_detected": metric_snapshot["anomalies_detected"],
        "normal_predictions": metric_snapshot["normal_predictions"],
        "average_reconstruction_error": sum(errors) / len(errors) if errors else 0,
        "max_reconstruction_error": max(errors) if errors else 0,
        "threshold": float(model.threshold) if model.is_trained else None,
...
def predict_single(request: PredictRequest):
    ...
    if not model.is_trained:
        raise HTTPException(status_code=503, detail="Model is not ready. Please retry shortly.")
    try:
...
async def predict_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"CSV prediction failed: {exc}") from exc

    recorded_alerts = []
    for result in results:
        # CSV files may optionally include these metadata columns; they are removed from model input by ml_model.py.
        metadata = PredictRequest(
            features={},
            source_ip=str(df.iloc[len(recorded_alerts)].get("sourceIP", df.iloc[len(recorded_alerts)].get("src_ip", "Unknown"))),
            destination_ip=str(df.iloc[len(recorded_alerts)].get("destinationIP", df.iloc[len(recorded_alerts)].get("dst_ip", "Unknown"))),
            protocol=str(df.iloc[len(recorded_alerts)].get("protocol", "Unknown")),
        )
        event = record_prediction(result, metadata)
        if event:
            recorded_alerts.append(event)

    return {
        "results": results,
        "alerts_created": recorded_alerts,
        "summary": {
            "total": len(results),
